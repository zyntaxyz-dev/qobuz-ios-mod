...
## 15. Test E2-restore: mecanismo OK, test NULO + esquema de streaming (2026-09-24, v6)
- `RESTORE from=session_export_init_1790256719.json added=0 updated=8 failed=0`, todo `st=0` (`logs/qobuz_hook.log:19-27`, contenedor 01F0 nuevo). El inyector funciona: 8 items sobrescritos en keychain sin crash.
- Pero el test es NULO: el export usado era **de la misma sesión** (epoch de hoy 8:31AM), no el de ayer (cuenta nueva). Se restauró la sesión sobre sí misma — no prueba trasplante. Requisito del test válido: baseline cuenta vieja (previews 30s) + `restore_request.txt` nombrando explícitamente el export de ayer + relaunch.
- Hallazgo colateral mayor: URLs de streaming en claro — `https://streaming-qobuz-std.akamaized.net/file?uid=…&eid=…&fmt=7&profile=raw&range=20-30&app_id=…&etsp=…&hmac=…` (`:47`, `:51`). Esquema: CDN Akamai, `fmt` = formato, `range=20-30` = preview de 30s (un full-track debería venir sin range o con rango completo — discriminador red del preview vs full). `hmac` en URL = bearer token hasta `etsp`: por eso el scrub v6.1 lo redacta (una URL full logueada sería replayable por cualquiera).
- CI: el commit del scrub falló por un `});` suelto (línea 98) y el fix pasó en verde (`a7d71b5`, run `36007381228` success, SK2 linked). Lección de proceso: el checker local de llaves no sustituye al clang del runner.
